...
$row = $admin->get_tgt(false,$_SESSION['ZTeam_adm']['admin_login']);
if($row === false || !isset($row['admin_master_id'])){
    cutil::redirect_exit("login.php");
}

if($row['admin_master_id'] != $_SESSION['ZTeam_adm']['admin_master_id']){
    cutil::redirect_exit("login.php");
}
if($row['admin_login'] != $_SESSION['ZTeam_adm']['admin_login']){
    cutil::redirect_exit("login.php");
}

function echo_hello_admin_name(){
    if(isset($_SESSION['ZTeam_adm']['admin_name'])){
